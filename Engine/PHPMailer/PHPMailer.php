...
class PHPMailer {
     ...
     *
     * @var integer @TODO Why is this needed when the SMTP class takes care of it?
     */
    public $Port = 25;
     ...
     *
     * @var array @TODO This should really not be public
     */
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name) {
        ...
        if (($pos = strrpos($address, '@')) === false) {
            ...
            if ($this->exceptions) {
                throw Util::HTTPException($error_message);
            }
            return false;
        }
        $params = array (
    ...
    public function preSend() {
        ...
        try {
            ...
            if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
                throw Util::HTTPException($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw Util::HTTPException($this->lang('empty_message'), self::STOP_CRITICAL);
            }
        ...
        if ($this->SingleTo) {
            foreach ( $this->SingleToArray as $toAddr ) {
                if (!@$mail = popen($sendmail, 'w')) {
                    throw Util::HTTPException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body) {
        ...
        if (!$result) {
            throw Util::HTTPException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
        return true;
    ...
    protected function smtpSend($header, $body) {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw Util::HTTPException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        if ('' == $this->Sender) {
            $smtp_from = $this->From;
        } else {
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw Util::HTTPException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw Util::HTTPException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw Util::HTTPException($this->lang('recipients_failed') . $errstr, self::STOP_CONTINUE);
        }
        return true;
    ...
    public function smtpConnect($options = array()) {
        ...
        foreach ( $hosts as $hostentry ) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw Util::HTTPException($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw Util::HTTPException($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate($this->Username, $this->Password, $this->AuthType, $this->Realm, $this->Workstation)) {
                            throw Util::HTTPException($this->lang('authenticate'));
                        }
                    }
                    return true;
                } catch ( \Exception $exc ) {
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw Util::HTTPException($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here, but need to wrap for PHP < 5.1
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw Util::HTTPException($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($file);
                    @unlink($signed);
                    throw Util::HTTPException($this->lang('signing') . openssl_error_string());
                }
            } catch ( \Exception $exc ) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment') {
        ...
        try {
            if (!@is_file($path)) {
                throw Util::HTTPException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64') {
        ...
        try {
            if (!is_readable($path)) {
                throw Util::HTTPException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
