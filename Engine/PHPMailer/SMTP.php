...
class SMTP {
    ...
    public function data($msg_data) {
        ...
        /*
         * The server is ready to accept data! According to rfc821 we should not send more than 1000 characters on a single line (including the CRLF) so we will break the data up into lines by \r and/or \n then if needed we will break each of those into smaller lines to fit within the limit. We will also look for lines that start with a '.' and prepend an additional '.'. NOTE: this does not count towards line-length limit.
         */
        ...
        /*
         * To distinguish between a complete RFC822 message and a plain message body, we check if the first field of the first line (':' separated) does not contain a space then it _should_ be a header and we will process all lines before a blank line as headers.
         */
     ...
     *            The data to send
     * @access public
     * @return integer boolean number of bytes sent to the server or false on error
     */
